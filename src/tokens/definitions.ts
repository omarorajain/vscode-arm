...
export enum TokenType {
  ...
  // ARM does not require :, but requires label to start at 0.
  Label = 1,
...
export enum TokenSubType {
  ...
  Declaration = 4, // label: {\n}.data_directive ('.byte, .word, .asciz, ...)
  Reference = 5, // Reference to a symbol or variable
  BeginMacro = 6, // .macro
  EndMacro = 7, // .endm
  BeginCondition = 8, // .if*
  EndCondition = 9, // .endif
  Include = 10, // .include
  MacroName = 11,
...
export interface Token extends TextRange {
  ...
  // Only set by tokenType the parser while building the AST. Clarifies if 'symbol' is 'instruction'
